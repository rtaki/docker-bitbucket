#!/bin/bash
isMod=$(grep "SSLCONNECTOR_INSERT" -c /opt/atlassian/atlassian-bitbucket-4.1.0/conf/server.xml)
...
if [ "${SSL}" = "TRUE" ]; then
 if [ "${KEYSTORE_PATH}" = "" ]; then
  ...
  /opt/atlassian/atlassian-bitbucket-4.1.0/conf/server.xml
...
/opt/atlassian/atlassian-bitbucket-4.1.0/bin/start-bitbucket.sh -fg
